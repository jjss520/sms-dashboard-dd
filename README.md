...
一个轻量级、自托管的短信接收和管理面板。配合 Android 短信转发器使用，可以方便地在网页端查看、管理转发的短信。

## ✨ 特性

- **单文件部署**：后端使用 Go 开发，前端构建产物通过 `embed` 嵌入，最终只生成一个二进制文件
- **移动端友好**：响应式设计，支持卡片式布局，手机端浏览体验佳
- **智能分组**：按设备机型自动分组，支持展开/收起，最新记录默认展示
...
- **参数说明**:
  ...
  - `sender`: 发件人号码（可选）
  - `phone`: 卡槽/手机号（可选，如：SIM2_1330000000）
